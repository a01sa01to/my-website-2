...
while i < len(file_content):
    new_content = {
        ...
        'status_4': file_content[i]['count'] if file_content[i]['status'] == 4 else 0,
    }
    for j in range(1, len(file_content)-i):
        if file_content[i]['date'] == file_content[i+j]['date']:
            ...
            elif file_content[i+j]['status'] == 4:
                new_content['status_4'] += file_content[i+j]['count']
        else:
            i += j-1
            break
...
with open('080004_ibaraki_covid19_vaccination.json', 'w', encoding="UTF-8") as f:
    json.dump(new_file_content, f, ensure_ascii=False,
              indent=2, separators=(',', ': '))
