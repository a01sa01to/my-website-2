<template>
      ...
      <ul>
        ...
        <li>
          <b>
            {{
              $t(
                'オミクロン株のデータについては、その検査方法の違いから、「検査陽性者数」は実際の陽性者数ではなく、あくまで「オミクロン株の可能性のある検体数」を示しています。'
              )
            }}
          </b>
        </li>
  ...
  covid19_ibaraki {
    mutant_test_people( [PAGINATION INFO] ) {
      dataset {
        date_from        <span class="token comment"># {{$t('検査実施期間の始点')}}: String!</span>
        date_to          <span class="token comment"># {{$t('検査実施期間の終点')}}: String!</span>
        ...
        tested           <span class="token comment"># {{$t('検査実施人数')}}: Int!</span>
        positive         <span class="token comment"># {{$t('検査陽性者数')}}: Int!</span>
